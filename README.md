# Flappy Bird – 20 Games Challenge

A fast–paced Flappy Bird–style game built in **Godot** as part of my 20 Games Challenge.  
Instead of a bird, you pilot a custom–made rocket plane, dodging obstacles and trying to survive as long as possible.  

All 3D assets were created in **Blender** and textured in **Substance Painter**, then polished with shaders, particles, and effects inside Godot.  

---

## Assets  

**Created by me (Blender + Substance Painter):**  
- Rocket Plane (main player)  
- Obstacles  
- Smoke particle effect  

**Third-party assets:**  
- [HDRI Skybox](https://freestylized.com/all-skybox/)  
- [Sounds](https://opengameart.org/)  
- [Water Shader](https://www.youtube.com/watch?v=7L6ZUYj1hs8)  
- [Outline Shader](https://godotshaders.com/shader/clean-pixel-perfect-outline-via-material-3/)  

---

## How to Play  

- Press **Spacebar** to make the rocket plane jump  
- Avoid hitting obstacles  
- Try to beat your high score  
